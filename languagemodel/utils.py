...
##
# Use this function
def batch_generator(ids, image_vectors,  batch_size, max_time):
    ...
    clip_image_len = len(image_vectors) - (len(image_vectors) % batch_size)

    # Yield batches
    word_indices = range(0, input_w.shape[1], max_time)
    img_indices = range(0, clip_image_len, batch_size)
    word_indices = word_indices[:len(img_indices)]
    for i,j in zip(word_indices, img_indices):
        yield input_w[:,i:i+max_time], image_vectors[j:j+batch_size], target_y[:,i:i+max_time]
