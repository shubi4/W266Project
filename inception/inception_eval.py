...
def _eval_once(saver, summary_writer, net, images, labels, summary_op):
  """Runs Eval once.
  ...
  with tf.Session() as sess:
    ...
    try:
      ...
      while step < num_iter and not coord.should_stop():
        ##top_1, top_5 = sess.run([top_1_op, top_5_op])
        net_val, labels_val, images_val = sess.run([net, labels, images])
        #print("net val type: %s, shape: %s,element type:%s" %(type(net_val), len(net_val), type(net_val[0])))
        print("net val shape:%s" %str(net_val.shape))
      ...
      print("Finished!!!")
      '''
      img_vectors =  np.vstack(all_net_val)
      print( img_vectors.shape)
      label_vectors = np.hstack(all_labels_val)
      print(label_vectors.shape)
      img_buffers = np.vstack(all_images_val)
      print(img_buffers.shape, img_buffers.size)
      print(label_vectors)
      print(img_vectors)


      np.savetxt("image_vectors.txt", img_vectors)
...
def evaluate(dataset):
  ...
  with tf.Graph().as_default():
    ...
    # inference model.
    #subha: modify return value
    net, _ = inception.inference(images, num_classes,restore_logits=False)
    ...
    while True:
      #Subha _eval_once(saver, summary_writer, top_1_op, top_5_op, summary_op)
      _eval_once(saver, summary_writer, net, images, labels, summary_op)
      if FLAGS.run_once:
        break
